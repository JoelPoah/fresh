...
export default function Transaction() {
  ...
  const handleCheckout = async (e) => {
    ...
    try {
      await axios.put(`https://k5ywx4t39h.execute-api.us-east-1.amazonaws.com/dev/image/checkout/1`, formData).then((response) => {
    ...
    try {
      await axios.put(`https://k5ywx4t39h.execute-api.us-east-1.amazonaws.com/dev/locker/checkout/1`, {product_name, product_gender, product_category, product_size, product_description}).then((response) => {
        const {result} = response.data
        setCheckoutSuccess(result)
        setCheckoutError('')
        navigate("/")
